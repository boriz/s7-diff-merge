...
namespace DotNetSiemensPLCToolBoxLibrary.Projectfiles
{
    public class Step7ProjectV5 : Project, IDisposable
    {
        ...
        */
        ...
        private List<HMIFolder> _hmiFolders;
        public List<HMIFolder> HMIFolders
        {
            get
            {
                if (!_projectLoaded)
        ...
        override internal void LoadProject()
        {
                ...
                foreach (DataRow row in dbfTbl.Rows)
                {
                    if (!(bool)row["DELETED_FLAG"] || _showDeleted)
                    {
                        if ((int)row["OBJTYP"] == 1314969 || (int)row["OBJTYP"] == 1314970 || (int)row["OBJTYP"] == 1315650)
                        {
                            ...
                            string ts1 = (string)row["CREATE"];
                            x.HW_Config_Created = Helper.GetDTFromListenTable((byte)ts1[0], (byte)ts1[1], (byte)ts1[2], (byte)ts1[3], (byte)ts1[4], (byte)ts1[5]);

                            ts1 = (string)row["MODIFIED"];
                            x.HW_Config_Modified = Helper.GetDTFromListenTable((byte)ts1[0], (byte)ts1[1], (byte)ts1[2], (byte)ts1[3], (byte)ts1[4], (byte)ts1[5]);

                            // Try to get corresponding HW config file
                            if (_ziphelper.FileExists(ProjectFolder + "hOmSave7" + _DirSeperator + "s7hstatx" + _DirSeperator + x.ID + ".s7h"))
                            {
                                x.HW_Config_Size = _ziphelper.FileSize(ProjectFolder + "hOmSave7" + _DirSeperator + "s7hstatx" + _DirSeperator + x.ID + ".s7h");
                            }
            ...
            // Get HMI stations
            if (_ziphelper.FileExists(ProjectFolder + "hOmSave7" + _DirSeperator + "wcf_stax" + _DirSeperator + "HOBJECT1.DBF"))
            {
                var dbfTbl = DBF.ParseDBF.ReadDBF(ProjectFolder + "hOmSave7" + _DirSeperator + "wcf_stax" + _DirSeperator + "HOBJECT1.DBF", _ziphelper, _DirSeperator);
                foreach (DataRow row in dbfTbl.Rows)
                {
                    if (!(bool)row["DELETED_FLAG"] || _showDeleted)
                    {
                        var x = new HMIFolder() { Project = this, Parent = ProjectStructure };
                        x.Name = ((string)row["Name"]).Replace("\0", "");
                        x.ID = (int)row["ID"];
                        x.UnitID = (int)row["UNITID"];
                        switch ((int)row["OBJTYP"])
                        {
                            case 19798113:
                                x.Type = HMIType.WinCCFlexible;
                                break;
                            default:
                                x.Type = HMIType.Unknown;
                                break;
                        }

                        string ts1 = (string)row["CREATE"];
                        x.Created = Helper.GetDTFromListenTable((byte)ts1[0], (byte)ts1[1], (byte)ts1[2], (byte)ts1[3], (byte)ts1[4], (byte)ts1[5]);

                        ts1 = (string)row["MODIFIED"];
                        x.Modified = Helper.GetDTFromListenTable((byte)ts1[0], (byte)ts1[1], (byte)ts1[2], (byte)ts1[3], (byte)ts1[4], (byte)ts1[5]);

                        this.HMIFolders.Add(x);
                    }
                }
            }
            ...
            if (_ziphelper.FileExists(ProjectFolder + "ombstx" + _DirSeperator + "offline" + _DirSeperator + "BSTCNTOF.DBF"))
            {
                ...
                foreach (DataRow row in dbfTbl.Rows)
                {
                    if (!(bool)row["DELETED_FLAG"] || _showDeleted)
                    {
                        ...
                        x.XutilsFolder = ProjectFolder + "XUTILS" + _DirSeperator;
                        x.XutilsBSFile = GetXUtilsFileForBlockFolder(x.XutilsFolder, x.ID);
        ...
        //JHC
        private string GetXUtilsFileForBlockFolder(String Folder, int ID)
        {
            String LinksFolder = Folder + "links" + _DirSeperator;
            
            if (!Directory.Exists(LinksFolder))
            {
                return null;
            }
            foreach (String file in Directory.EnumerateFiles(LinksFolder))
            {
                if ((Regex.Match(file.ToUpper(), @"BSL").Success) && file.ToUpper().EndsWith(".DBF"))
                {
                    var dbfTbl = DBF.ParseDBF.ReadDBF(file, _ziphelper, _DirSeperator);
                    foreach (DataRow row in dbfTbl.Rows)
                    {
                        if (!(bool)row["DELETED_FLAG"])
                        {
                            if ((int)row["TOI"] == ID)
                            {
                                String fileName = file.Substring(file.LastIndexOf(_DirSeperator));
                                int XutilsID = Convert.ToInt32(Regex.Match(fileName, @"(\d+)").ToString());
                                return "listen" + _DirSeperator + "BS" + XutilsID.ToString().PadLeft(6, '0') + ".DBF";
                            }
                        }
                    }
                }
