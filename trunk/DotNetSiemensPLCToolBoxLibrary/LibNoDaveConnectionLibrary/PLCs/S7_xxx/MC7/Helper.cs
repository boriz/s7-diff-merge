...
namespace DotNetSiemensPLCToolBoxLibrary.PLCs.S7_xxx.MC7
{
    ...
    public static class Helper
    {
        ...
        static public long GetIntFromHexString(string myString)
        {
            long val = 0;
        ...
        public static string GetDaT(UInt32 LW1, UInt32 LW2)
        {
            string Result = "";

            byte[] b1 = BitConverter.GetBytes(LW1);
            byte[] b2 = BitConverter.GetBytes(LW2);
        ...
        public static DateTime GetDT(byte b1, byte b2, byte b3, byte b4, byte b5, byte b6)
        {
            ...
            long lMs = (b1 * 0x1000000) + (b2 * 0x10000) + (b3 * 0x100) + b4;
            DT = DT.AddMilliseconds(lMs);
            long lDays = (b5 * 0x100) + b6;
            DT = DT.AddDays(lDays);
        ...
        */
        ...
        internal static DateTime GetDTFromListenTable(byte b1, byte b2, byte b3, byte b4, byte b5, byte b6)
        {
            System.DateTime DT;
            string Result;

            DT = new DateTime(1984, 1, 1, 0, 0, 0, 0);
            UInt64 lMs = (UInt64) ((b1 * 0x1000000) + (b2 * 0x10000) + (b3 * 0x100) + b4);
            DT = DT.AddMilliseconds(lMs);
            UInt64 lDays = (UInt64)((b5 * 0x100) + b6);
            DT = DT.AddDays(lDays);
          //  DT = DT.AddDays(131);
