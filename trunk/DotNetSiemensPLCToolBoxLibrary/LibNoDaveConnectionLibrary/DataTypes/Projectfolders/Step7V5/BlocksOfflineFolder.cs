...
namespace DotNetSiemensPLCToolBoxLibrary.DataTypes.Projectfolders.Step7V5
{
    ...
    {
        //JHC adding xutils folder for proper block list & datetime retrieval
        public string XutilsFolder { get; set; }
        public string XutilsBSFile { get; set; }
        public string Folder { get; set; }

        //JHC adding link DBF dataTable for proper block list & datetime retrieval\
        private DataTable _xutilsDbf;
        private DataTable xutilsDBF
        {
            get
            {
                if (_xutilsDbf == null)
                    if (XutilsBSFile != null)
                        if (((Step7ProjectV5)Project)._ziphelper.FileExists(XutilsFolder + XutilsBSFile))
                            _xutilsDbf = DBF.ParseDBF.ReadDBF(XutilsFolder + XutilsBSFile, ((Step7ProjectV5)Project)._ziphelper,
